<script lang="ts">
  ...
  const filteredStudies = $derived(
    activeTag ? data.caseStudies.filter(p => p.tags.includes(activeTag!)) : data.caseStudies,
  );
  const visibleItems = $derived(filteredStudies.slice(0, visibleCount));
  const hasMore = $derived(filteredStudies.length > visibleCount);
  const remaining = $derived(filteredStudies.length - visibleCount);
  ...
  const itemCount = $derived(filteredStudies.length);
  const itemWord = $derived(itemCount === 1 ? "case study" : "case studies");

  const jsonLd = $derived(`<script type="application/ld+json">${JSON.stringify({
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    "name": `Case Studies — ${config.title}`,
    "description": `Case studies by Steele S. — ${data.caseStudies.length} deep dives into architecture and design decisions.`,
    "url": `${config.url}case-studies`,
    "numberOfItems": data.caseStudies.length,
  })}<\/script>`);
...
<svelte:head>
  <title>Case Studies — {config.title}</title>
  <meta name="description" content={`Case studies by Steele S. — ${data.caseStudies.length} deep dives into architecture and design decisions.`} />
  <meta property="og:type" content="website" />
  <meta property="og:title" content={`Case Studies — ${config.title}`} />
  <meta property="og:description" content={`Case studies by Steele S. — ${data.caseStudies.length} deep dives into architecture and design decisions.`} />
  <meta property="og:url" content={`${config.url}case-studies`} />
  <meta property="og:image" content={config.ogImageUrl({ title: "Case Studies", category: "case-study", subtitle: `${data.caseStudies.length} studies` })} />
  <meta property="og:image:width" content="1200" />
  <meta property="og:image:height" content="630" />
  <meta property="og:image:alt" content={`Case Studies — ${config.title}`} />
  <meta name="twitter:card" content="summary_large_image" />
  <meta name="twitter:image" content={config.ogImageUrl({ title: "Case Studies", category: "case-study", subtitle: `${data.caseStudies.length} studies` })} />
  <link rel="canonical" href={`${config.url}case-studies`} />
...
<div class="listing-page">
  ...
  <div class="listing-page__inner">
    <header class="listing-page__header">
      <h1 class="listing-page__title">Case Studies</h1>
      <p class="listing-page__count">
        {itemCount} {itemWord}
      </p>
      <p class="listing-page__subtitle">Deep dives into architecture and design decisions.</p>
    </header>
    ...
    <ul class="listing-page__list" role="list">
      ...
      {:else}
        <li class="listing-page__empty">No case studies found.</li>
      {/each}
    </ul>
